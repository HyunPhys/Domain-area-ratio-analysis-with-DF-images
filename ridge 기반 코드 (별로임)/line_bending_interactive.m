...
opts = setDefault(opts,'ridgeEnable', true);      % 리지드 경로 보정 on/off
opts = setDefault(opts,'ridgeSigmas', [1 2 3]);   % vesselness 멀티스케일
opts = setDefault(opts,'ridgeCorridor', 12);      % chord 주변 탐색 폭(px)
opts = setDefault(opts,'ridgeRefineWin', 2);      % 서브픽셀 정렬 윈도우(±win)
opts = setDefault(opts,'ridgePick','vesselness'); % 'vesselness' or 'length'
...
for k=1:size(pairs,1)
    ...
    if opts.ridgeEnable
        pathRC_skel = pathRC;  % 백업

        % 리지드 경로 추출 + 정렬
        pathRC_ridge = findPathRidge(Igray, p1_rc, p2_rc, opts);
        pathRC_ridge = refinePathByNormal(Igray, pathRC_ridge, p1_rc, p2_rc, opts.ridgeRefineWin);

        % 두 경로 중 선택 기준
        if strcmpi(opts.ridgePick,'vesselness')
            Vtest = vesselness2d(im2double(Igray), opts.ridgeSigmas);
            score = @(P) mean( Vtest(sub2ind(size(Vtest), P(:,1), P(:,2))) );
            if score(pathRC_ridge) > score(pathRC_skel)
                pathRC = pathRC_ridge;
            else
                pathRC = pathRC_skel;
            end
        else  % 'length' : 픽셀 길이가 더 짧은 경로
            if size(pathRC_ridge,1) < size(pathRC_skel,1)
                pathRC = pathRC_ridge;
            else
                pathRC = pathRC_skel;
            end
